#include "snc.h"

static const snByte SNC_sbox[256] = {
    
};

static const snByte SNC_rsbox[256] = {
    
};

#define SNC_SBOX(x)  (SNC_sbox[(x)])
#define SNC_RSBOX(x) (SNC_rsbox[(x)])

static snVoid SNC_keyExtension(snByte *iv, snByte *key)
{
    static snSize_t i;
    static snByte buf;

    for(i = 0; i < SNC_BLOCKLEN; ++i) {
        buf = 
            key[i]                       ^ key[(i + 1)  % SNC_BLOCKLEN] ^
            key[(i + 4)  % SNC_BLOCKLEN] ^ key[(i + 5)  % SNC_BLOCKLEN] ^
            key[(i + 8)  % SNC_BLOCKLEN] ^ key[(i + 9)  % SNC_BLOCKLEN] ^
            key[(i + 12) % SNC_BLOCKLEN] ^ key[(i + 13) % SNC_BLOCKLEN] ^
            key[(i + 16) % SNC_BLOCKLEN] ^ key[(i + 17) % SNC_BLOCKLEN] ^
            key[(i + 20) % SNC_BLOCKLEN] ^ key[(i + 21) % SNC_BLOCKLEN] ^
            key[(i + 24) % SNC_BLOCKLEN] ^ key[(i + 25) % SNC_BLOCKLEN] ^
            key[(i + 28) % SNC_BLOCKLEN] ^ key[(i + 29) % SNC_BLOCKLEN] ^
            iv[(i + 7) % SNC_BLOCKLEN]   ^ iv[(i + 16) % SNC_BLOCKLEN];

        iv[i] ^= buf;

        buf = 
            key[(i + 2)  % SNC_BLOCKLEN] ^ key[(i + 3)  % SNC_BLOCKLEN] ^
            key[(i + 6)  % SNC_BLOCKLEN] ^ key[(i + 7)  % SNC_BLOCKLEN] ^
            key[(i + 10) % SNC_BLOCKLEN] ^ key[(i + 11) % SNC_BLOCKLEN] ^
            key[(i + 14) % SNC_BLOCKLEN] ^ key[(i + 15) % SNC_BLOCKLEN] ^
            key[(i + 18) % SNC_BLOCKLEN] ^ key[(i + 19) % SNC_BLOCKLEN] ^
            key[(i + 22) % SNC_BLOCKLEN] ^ key[(i + 23) % SNC_BLOCKLEN] ^
            key[(i + 26) % SNC_BLOCKLEN] ^ key[(i + 27) % SNC_BLOCKLEN] ^
            key[(i + 29) % SNC_BLOCKLEN] ^ key[(i + 31) % SNC_BLOCKLEN] ^
            iv[(i + 7) % SNC_BLOCKLEN]   ^ iv[(i + 16) % SNC_BLOCKLEN];

        iv[(i + 17) % SNC_BLOCKLEN] ^= buf;

        key[(i + 7)  % SNC_BLOCKLEN] = (buf ^ key[(i + 7)  % SNC_BLOCKLEN]) ^ (SNC_BLOCKLEN % (i + 1));
        key[(i + 14) % SNC_BLOCKLEN] = (buf ^ key[(i + 14) % SNC_BLOCKLEN]) + (SNC_BLOCKLEN % (i + 1));
        key[(i + 21) % SNC_BLOCKLEN] = (buf ^ key[(i + 21) % SNC_BLOCKLEN]) + (SNC_BLOCKLEN % (i + 1));
        key[(i + 28) % SNC_BLOCKLEN] = (buf ^ key[(i + 28) % SNC_BLOCKLEN]) + (SNC_BLOCKLEN % (i + 1));
        key[(i + 35) % SNC_BLOCKLEN] = (buf ^ key[(i + 35) % SNC_BLOCKLEN]) + (SNC_BLOCKLEN % (i + 1));
        key[(i + 42) % SNC_BLOCKLEN] = (buf ^ key[(i + 42) % SNC_BLOCKLEN]) + (SNC_BLOCKLEN % (i + 1));

        key[i] = buf;
    }
}

// --------------------------------------------------------------------
snVoid SNC_init_ctx(SNC_ctx *ctx, const snByte *keyBuf)
{
    static sn_u32 r;
    static snByte key[SNC_BLOCKLEN];
    static snByte iv[SNC_BLOCKLEN];

    memcpy(key, keyBuf, SNC_BLOCKLEN);
    memcpy(iv, ctx->iv, SNC_BLOCKLEN);

    for(r = 0; r < SNC_NR; ++r) {
        memcpy(ctx->rk[r], key, SNC_BLOCKLEN);
        SNC_keyExtension(iv, key);
    }
}
